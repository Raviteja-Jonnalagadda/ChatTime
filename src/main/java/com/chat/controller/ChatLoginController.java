...
public class ChatLoginController {
	...
	@Autowired
	ChatTimeCoreUtility cu;
	@PostMapping("/ChangePassword")
	public String PasswordUpdate(@RequestParam("mail_id") String email, @RequestParam("new_otp") String otp,@RequestParam("pword") String pword) {
		if (email.isEmpty() || otp.isEmpty() || pword.isEmpty()) {
			return "NULVAL";
		}
		String updatestatus = cu.PasswordUpdate(email, otp, pword);
		System.out.println("[ChatCoreController] [PasswordUpdate] The update status is -->  " + updatestatus);
		return updatestatus;
